...
export default function SignatureModal({ isOpen, onClose, onSuccess, documentId, darkMode }: SignatureModalProps) {
        ...
        if (!isOpen) {
            ...
            const checkIdentity = async () => {
                ...
                try {
                    ...
                    setIsVerified(!!res.data.isVerified || !!res.data.isIdentityVerified);
                } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        // Only require drawn signature if not verified
        if (!isVerified) {
        ...
        try {
            ...
            if (!isVerified) {
                ...
                // If verified, bypass proxy and just sign
                await api.post(`/documents/${documentId}/sign`, { 
                    stampType
                });
            }
                ...
                <div className="p-6 flex flex-col items-center">
                    {loadingIdentity ? (
                        ...
                        <>
                            ...
                            {isVerified ? (
                                <div className="text-center w-full mb-6 py-8">
                                    <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <CheckCircle className="w-8 h-8 text-green-600" />
                                    </div>
                                    <h4 className={`text-lg font-bold mb-2 ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>Identity Pre-Verified</h4>
                                    <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                        You have already completed your identity verification. Your master signature will be applied to this document.
                                    </p>
                                </div>
                            ) : (
                                <>
                                    {/* Video Area (Always visible if not verified) */}
                    ...
                    {(videoBlob || isVerified) && (
                        <div className="w-full flex flex-col gap-4 mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className="flex gap-4">
                                ...
                                <div className="flex-1">
                                    {/* Handwritten Signature Area (Only if not verified) */}
                                    {!isVerified && (
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className={`border-2 border-dashed rounded-xl overflow-hidden cursor-crosshair transition-colors ${
